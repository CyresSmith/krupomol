...
const Page = async () => {
    ...
    return productsToRender.length > 0 ? (
        <div className="rounded-20 bg-primary px-4 py-4 text-background shadow-lg desktop:rounded-40 desktop:px-24 desktop:py-12">
            <Title
                className="border-b-2 border-background pb-4 font-title font-bold mobile:text-3xl tablet:text-3xl"
                title="Закупівельні ціни"
            />
            <Accordion className="mobile:pr-0" collapsible type="single">
                {productsToRender.map(product => (
                    <AccordionItem key={product.productName} value={product.productName}>
                        <AccordionTrigger className="justify-between font-title text-xl font-bold mobile:text-lg">
                        ...
                        <AccordionContent className="mb-4 rounded-20 bg-background p-5 text-base mobile:p-3">
